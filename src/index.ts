...
import { getPostCSSWatchFiles, RenderOptions, renderStyle } from "./utils";
...
const onTempStyleResolve = async (
    build: PluginBuild,
    args: OnResolveArgs,
...
const onStyleResolve = async (
    build: PluginBuild,
    args: OnResolveArgs,
): Promise<OnResolveResult | null> => {
    const { namespace } = args;
    if (
        args.pluginData === SKIP_RESOLVE ||
...
const onStyleLoad = (options: PluginOptions) => {
    return async (args: OnLoadArgs): Promise<OnLoadResult> => {
        ...
        if (isCSSModule) {
            ...
            plugins = [
                handleCSSModules(mapping, cssModulesOptions),
                ...plugins,
            ];
            injectMapping = true;
        }
        ...
        if (extract) {
            ...
            contents += `import ${JSON.stringify(
                "ni:sha-256;" +
                    createHash("sha256").update(css).digest("base64url"),
            )};`;
        }
...
const stylePlugin = (options: PluginOptions = {}) => ({
    ...
    setup: async (build: PluginBuild) => {
        ...
        build.onResolve(
            { filter: styleFilter },
            onStyleResolve.bind(null, build),
        );
        build.onResolve(
            { filter: /^ni:/, namespace: LOAD_STYLE_NAMESPACE },
            onTempStyleResolve.bind(null, build),
        );

        // New temp files from rendered css must be evaluated
        build.onLoad(
            { filter: /.*/, namespace: LOAD_TEMP_NAMESPACE },
            onTempLoad,
        );

        // Render css with CSS Extensions / Preprocessors and PostCSS
        build.onLoad(
            { filter: /.*/, namespace: LOAD_STYLE_NAMESPACE },
            onStyleLoad(options),
        );
    },
});
